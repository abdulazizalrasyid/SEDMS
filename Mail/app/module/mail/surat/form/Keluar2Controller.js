Ext.define('Docs.view.surat.form.Keluar2Controller', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.surat-form-keluar2',
    init:function(){
        me=this;
        ...
        me.lookupReference('unit_pemrakarsa').setStore(this.getViewModel().getStore('unit_pemrakarsa'));
    },  
                    ...
                    //start process
                        if (form.isValid()) {
                            Ext.Msg.confirm('Konfirmasi', 'Apakah anda yakin akan menyimpan data ini?',
                                function(btn) {
                                    if (btn === 'yes') {
                                        form.url = serverURL+'surat/simpan_4';
                                        form.submit({
                                            params: {
                                                tahun: tahunmasuk,
                                                tipe_surat:'K',
    ...
    onPenandatanganSelect:function(me, records, eOpts){
        ...
        this.lookupReference('penandatangan2').setValue(records[0].data.jabatan);
        //this.lookupReference('unit_pemrakarsa').setValue(records[0].data.kode_unker);
        //this.setNomorSurat();
    },
    onPenandatanganChange:function(self, newValue, oldValue, eOpts ){
        this.lookupReference('penandatangan2').setValue(newValue);
    },
    setNomorSurat:function(){
        ...
        nomor = me.lookupReference('no_registrasi_surat').getValue();
        kode_jabatan = me.lookupReference('kode_jabatan').getValue();
        if (jenis == 'KEP'){
